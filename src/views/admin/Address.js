  ...
  return (
    <>
      ...
      <Column style={{ paddingTop: 15, width: '100%', flexDirection: 'row', justifyContent: 'space-around', }} >
        <Button outline bgcolor={!p.item.queueSend ? 'orange' : 'red'} style={{ backgroundColor: '#f7f7f7', height: 30 }} onClick={() => { postQueue(p.item._id); }} > {!p.item.queueSend ? 'در صف ارسال ' : 'خروج از صف'}</Button>
        <Button outline bgcolor='green' style={{ backgroundColor: '#f7f7f7', height: 30 }} onClick={() => { postedOrder(p.item._id) }} >ارسال شد</Button>
        {p.item.latlng?.lat ? <Button outline bgcolor='blue' style={{ backgroundColor: '#f7f7f7', height: 30 }} onClick={() => { p.navigation.navigate('ShowLatLngOnMap', { latlng: p.item.latlng }) }} >نمایش</Button> : <></>}
      </Column>
    </>
  )
}
